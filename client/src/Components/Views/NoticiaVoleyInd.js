...
const NoticiaVoleylInd = () => {
  ...
  const [NoticiaVoleyImg, setNoticiaVoleyImg] = useState("");
  const [Fuente, setFuente] = useState("");
  const getNoticiaVoleyIndividual = async () => {
    await db
      ...
      .then((doc) => {
        if (doc.exists) {
          ...
          setNoticiaVoleyImg(doc.data().NoticiaVoleyImg);
          setFuente(doc.data().Fuente);
          console.log("Document data:", Body, NoticiaVoleyImg);
        } else {
...
export default NoticiaVoleylInd;
